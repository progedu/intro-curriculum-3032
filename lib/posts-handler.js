  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        console.log("箱の中みたい" + decoded);//id=9&oneTimeToken=bae3edcf6a8bba26
        const dataArray = decoded.split('&');
        console.log("中みたい" + dataArray);//id=12,oneTimeToken=e170480c119d2e3f
        const id = dataArray[0] ? dataArray[0].split('id=')[1] : '';  //["", 数字]となる
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : '';
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
