...
const postOTPs = new Map(); // キーをユーザー名、値をトークンとする連想配列
const deleteOTPs = new Map(); // 削除時のワンタイムトークン
  ...
  switch (req.method) {
    case 'GET':
      ...
      Post.findAll({ order: 'id DESC' }).then((posts) => {
        ...
        const postOTP = crypto.randomBytes(8).toString('hex');
        const deleteOTP = crypto.randomBytes(8).toString('hex');
        postOTPs.set(req.user, postOTP);
        deleteOTPs.set(req.user, deleteOTP);
    ...
    case 'POST':
      req.on('data', (data) => {
        ...
        const requestedPostOTP = dataArray[1] ? dataArray[1].split('postOTP=')[1] : '';
        if (postOTPs.get(req.user) === requestedPostOTP) {
  ...
  switch (req.method) {
    case 'POST':
      req.on('data', (data) => {
        ...
        const id = params[0] ? params[0].split('id=')[1] : '';
        const deleteOTP = params[1] ? params[1].split('deleteOTP=')[1] : '';

        Post.findById(id).then((post) => {
          if( req.user === post.postedBy || req.user === 'admin' ) {

            if(deleteOTPs.get(req.user) === deleteOTP) {
              deleteOTPs.delete(req.user);
              post.destroy();
              console.info(
