  ...
  switch (req.method) {
    case 'GET':
      ...
      Post.findAll({ order: [['id', 'DESC']] }).then((posts) => {
        ...
        const deleteoneTimeToken = crypto.randomBytes(8).toString('hex');
        deleteTokenMap.set(req.user,deleteoneTimeToken);
        res.end(pug.renderFile('./views/posts.pug', {
          posts: posts,
          user: req.user,
          oneTimeToken: oneTimeToken,
          deleteoneTimeToken:deleteoneTimeToken
        }));
        console.info(
  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const id = Array ? Array[0].split('id=')[1] : '';
        const postedcont = id ? Array[1] : ''; 
        const deloneToken = postedcont ? postedcont.split('oneTimetoken=')[1] : '';
        if(deloneToken === deleteTokenMap.get(req.user)){
