  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : '';
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) { // トークンが正当な場合
          Post.findById(id).then((post) => {
            if (req.user === post.postedBy || req.user === 'admin') { // 投稿削除権限(投稿者本人or管理人)の再確認
              post.destroy();
              console.info(
            ...
            oneTimeTokenMap.delete(req.user); // 利用済みトークンを削除
            handleRedirectPosts(req, res); // 処理終了後、元のページへリダイレクト
          });
        } else { // トークンが不正な場合
          util.handleBadRequest(req, res);
        }
