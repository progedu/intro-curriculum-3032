  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        // 投稿内容の改行に対応 ただし何もなしの投稿も通してしまう
        const matchResult = decoded.match(/content=([\s\S]*)&oneTimeToken=(.*)/);
        // console.log(`matchの結果は${matchResult}です`)
        if (!matchResult) {
  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        // console.log(`Delete時のdecodedの結果は${decoded}です`)
        const matchResult = decoded.match(/id=(.*)&oneTimeToken=(.*)/);
        // console.log(`Delete時のmatchの結果は${matchResult}です`)
        if(!matchResult) {
          util.handleBadRequest(req, res);
        } else {
