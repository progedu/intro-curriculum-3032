  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const dataArray = decoded.split('&');
        const content = dataArray[0] ? dataArray[0].split('content=')[1] : '';
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=') : '';
        const id = decoded.split('id=')[1];
        Post.findById(id).then((post) => {
          if ((req.user === post.postedBy || req.user === 'admin') && oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
            post.destroy();
            console.info(
