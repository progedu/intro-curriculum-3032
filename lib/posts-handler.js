  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        const dataArray = decoded.split('&');//idとoneTimeTokenを分割
        const id = dataArray[0] ? dataArray[0].split('id=')[1] : '';//idを取り出し
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : ''; //oneTimeTokenを取り出し
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) { //oneTimeTokenが有効なら削除を実行
            ...
            if (req.user === post.postedBy || req.user === 'admin') {
              post.destroy().then(() => {
                console.info(
                  ...
                  `remoteAddress: ${req.connection.remoteAddress}, ` +
                  `userAgent: ${req.headers['user-agent']} ` +
                  `oneTimeToken: ${requestedOneTimeToken} `
                );
                oneTimeTokenMap.delete(req.user);//使用済みトークンを削除
                handleRedirectPosts(req, res);
              });
            }
          });
        } else { //oneTimeTokenが無効ならエラー
          util.handleBadRequest(req, res);
        }
      });
      break;
