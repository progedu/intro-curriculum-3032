    ...
    case 'GET':
      ...
      */
      ...
      let renderFunc = function (ps) {
        ...
        // 8byteのワンタイムトークンを生成する
        const oneTimeToken = crypto.randomBytes(8).toString('hex');
        // ユーザー名をキーとしたハッシュを設定する
    ...
    case 'POST':
      ...
      */
      let endFunc = function () {
        ...
        // 投稿内容の部分がfalsyでなければ
        if (dataArray[0]) {
          // content=XXXXXXとなっているのでsplitでXXXXXXの部分を取得する
          content = dataArray[0].split('content=')[1];
        }
        // ワンタイムトークンを保存する変数
        let requestedOneTimeToken = '';
        // 同様にワンタイムトークンの部分がfalsyでなければ
        if (dataArray[1]) {
          // oneTimeToken=YYYYYYYYYYとなっているのでYYYYYYYYYYの部分を取得する
          requestedOneTimeToken = dataArray[1].split('oneTimeToken=')[1];
        }
        // ハッシュテーブルに登録されているトークンと
        // リクエストされてきたトークンを比較することで
        // 正しいフォームから送られてきたリクエストか確認することができる
        // 正しいトークン情報であれば
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
          ...
          /** 
           * 正常に投稿できたあとに実行する関数。
           * ハッシュテーブルから投稿ユーザーのトークンを削除し、/postsへリダイレクトする
           * （リダイレクト直後に新たなトークンが生成されるので、削除して問題ない。
           * 　むしろ削除しなきゃダメ）
          */
          const deleteTokenAndRedirectPostsFunc = function () {
            // ハッシュテーブルから投稿ユーザーのワンタイムトークンを削除する
            oneTimeTokenMap.delete(req.user);
            // /postsへリダイレクトする。直後に新たなワンタイムトークンが生成される。
            handleRedirectPosts(req, res);
          }
          // Postモデルが正常に作成されたらワンタイムトークンを削除したり、
          // リダイレクトしたり事後処理を行う
          post.then(deleteTokenAndRedirectPostsFunc);
        }
        //正しいトークンでなければ
        else {
          // BadRequest処理を行う
    ...
    case 'POST':
      ...
      */
      let endFunc = function () {
        ...
        const decoded = decodeURIComponent(body);
        // console.log('decoded = ' + decoded);
        // decodedは'id=XX&oneTimeToken=YYYYYYYYYY'
        // という形式になっているのでまず'&'で分割してやる
        const dataArray = decoded.split('&');
        // 削除したい投稿のIDを保存する変数
        let id = '';
        // 削除したい投稿のIDの部分がfalsyでなければ
        if (dataArray[0]) {
          // id=XXXXXXとなっているのでsplitでXXXXXXの部分を取得する
          id = dataArray[0].split('id=')[1];
        }
        // ワンタイムトークンを保存する変数
        let requestedOneTimeToken = '';
        // 同様にワンタイムトークンの部分がfalsyでなければ
        ...
        // console.log('id = ' + id);
        // console.log('token = ' + requestedOneTimeToken);
        // ハッシュテーブルに登録されているトークンと
        // リクエストされてきたトークンを比較することで
        // 正しいフォームから送られてきたリクエストか確認することができる
        // 正しいトークン情報であれば
