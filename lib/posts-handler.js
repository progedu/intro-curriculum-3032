  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        const idTokenArray = decoded.split('&');
        const id = idTokenArray[0] ? idTokenArray[0].split('id=')[1] : '';
        const requestedOneTimeToken = idTokenArray[1] ? idTokenArray[1].split('oneTimeToken=')[1] : '';
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
