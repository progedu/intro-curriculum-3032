  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const id = splited[0].split('id=')[1];
        const delKey=splited[1]? splited[1].split('=')[1]:"";
        Post.findByPk(id).then((post) => {
          if(oneTimeTokenMap.get(req.user)===delKey){
            if (req.user === post.postedBy || req.user === 'admin') {
              post.destroy().then(() => {
                console.info(
                  ...
                  `remoteAddress: ${req.connection.remoteAddress}, ` +
                  `userAgent: ${req.headers['user-agent']} `
                  //oneTimeTokenMap.get(req.user)
                );
                handleRedirectPosts(req, res);
              });
            }
          }else{util.handleBadRequest(req, res);}
        });
      });
      break;
    default:
