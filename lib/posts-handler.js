  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        const deleteArray = decoded.split('&');
        const id = deleteArray[0] ? deleteArray[0].split('id=')[1] : '';
        const requestedOneTimeToken = deleteArray[1] ? deleteArray[1].split('oneTimeToken=')[1] : '';
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
