...
function getRequestedOneTimeToken(body,what){
  body = Buffer.concat(body).toString();
  const decoded = decodeURIComponent(body);
  const dataArray = decoded.split('&');
  const splitName = what;
  what = dataArray[0] ? dataArray[0].split(`${splitName}=`)[1] : '';
  const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : '';
  return requestedOneTimeToken;
}
  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        if (oneTimeTokenMap.get(req.user) === getRequestedOneTimeToken(body,content)) {
  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const id = "id";
        if (oneTimeTokenMap.get(req.user) === getRequestedOneTimeToken(body,id)) {
