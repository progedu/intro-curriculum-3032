  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        const id_token = decoded.match(/id=(.*)&oneTimeToken=(.*)/);
        console.log('id_token: ' + id_token);
        if (!id_token) {
          util.handleBadRequest(req, res);
        }
        else {
          console.log('id_token[1]: ' + id_token[1] + 'id_token[2]: ' + id_token[2]);
          const id = id_token[1];
          const oneTimeToken = id_token[2];
          if (oneTimeToken === oneTimeTokenMap.get(req.user)) {
