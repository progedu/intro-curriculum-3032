  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const decorded = decodeURIComponent(body);
        const dataArray = decorded.split('&');
        const id = dataArray[0] ? dataArray[0].split('id=')[1] : '';
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : '';
        if (oneTimeTokenMap.get(req, res) === requestedOneTimeToken) {
           ...
           if (req.user === post.postedBy || req.user === 'admin') {
              post.destory().then(() => {
                console.info(
                  ...
                  `remoteAddress: ${req.connection.remoteAddress}, ` +
                  `userAgent: ${req.handers['user-agent']} `
                );
