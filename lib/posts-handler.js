  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].decoded.split('oneTimeToken=')[1] : '';
        if (requestedOneTimeToken === oneTimeTokenMap.get(req.user)) {
          ...
          handle.handleBadRequest(req, res);
        }
      });
      break;
    default:
